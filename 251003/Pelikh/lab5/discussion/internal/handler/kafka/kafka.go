...
func NewKafkaConsumer(brokers []string, groupID string, noticeService service.NoticeService) *KafkaConsumer {
...
func (k *KafkaConsumer) processNotice(ctx context.Context) {
...
func (k *KafkaConsumer) processRequest(ctx context.Context, request struct {
	...
	case "create":
		notice, err := k.noticeService.CreateNotice(ctx, model.Notice{
	...
	case "get":
		notice, err := k.noticeService.GetNotice(ctx, request.ID)
		if err != nil {
			response = map[string]interface{}{"error": err.Error()}
		} else {
			response = map[string]interface{}{"message": notice}
	...
	case "get_all":
		notices, err := k.noticeService.GetNotices(ctx)
		if err != nil {
			response = map[string]interface{}{"error": err.Error()}
		} else {
	...
	case "update":
		notice, err := k.noticeService.UpdateNotice(ctx, model.Notice{
			ID:      request.ID,
			TopicID: request.IssueID,
			Content: request.Content,
		})
	...
	case "delete":
		err := k.noticeService.DeleteNotice(ctx, request.ID)
		if err != nil {
			response = map[string]interface{}{"error": err.Error()}
		} else {
