...
func New(srv Service, markService mark.Service) topic {
	return topic{
		srv:         srv,
...
func (i topic) create(w http.ResponseWriter, r *http.Request) {
	...
	if req.Marks != nil {
		...
		for _, l := range req.Marks {
			_, err := i.markService.Create(r.Context(), model.Mark{Name: l, ID: int64(rand.Intn(math.MaxInt) + 1)})
			if err != nil {
