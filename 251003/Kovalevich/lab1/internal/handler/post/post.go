...
import (
	...
	"errors"
	"fmt"
	commentErr "github.com/Khmelov/Distcomp/251003/truhan/lab1/internal/storage/post"
	"net/http"
	"strconv"
...
type Service interface {
	Create(ctx context.Context, comment model.Post) (model.Post, error)
	GetList(ctx context.Context) ([]model.Post, error)
	Get(ctx context.Context, id int64) (model.Post, error)
	Update(ctx context.Context, comment model.Post) (model.Post, error)
...
func (h Handler) get(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, commentErr.ErrPostNotFound) {
