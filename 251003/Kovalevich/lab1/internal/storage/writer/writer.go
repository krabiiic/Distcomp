...
func (r repo) Create(ctx context.Context, req model.Writer) (model.Writer, error) {
	query := `INSERT INTO writer (login, password, firstname, lastname) 
...
func (r repo) Get(ctx context.Context, id int64) (model.Writer, error) {
	...
	query := `SELECT id, login, password, firstname, lastname FROM writer WHERE id = $1`
...
func (r repo) Update(ctx context.Context, req model.Writer) (model.Writer, error) {
	...
	query := `UPDATE writer SET login = $1, password = $2, firstname = $3, lastname = $4 
...
func (r repo) Delete(ctx context.Context, id int64) error {
	query := `DELETE FROM writer WHERE id = $1`
