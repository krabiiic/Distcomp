...
type kafkaResponse struct {
	Post  *PostModel.Post  `json:"Post,omitempty"`
	Posts []PostModel.Post `json:"Posts,omitempty"`
	Error string           `json:"error,omitempty"`
}

func (k *KafkaClient) CreatePost(ctx context.Context, topicID int64, content string) (*PostModel.Post, error) {
	req := kafkaRequest{
		Action:    "create",
		topicID:   topicID,
		Content:   content,
...
func (k *KafkaClient) GetPosts(ctx context.Context) ([]PostModel.Post, error) {
	req := kafkaRequest{
		Action:    "get_all",
		ReplyTo:   k.responseTopic,
		Timestamp: time.Now().Unix(),
...
func (k *KafkaClient) GetPost(ctx context.Context, id int64) (*PostModel.Post, error) {
	req := kafkaRequest{
		Action:    "get",
		ID:        id,
		ReplyTo:   k.responseTopic,
...
func (k *KafkaClient) UpdatePost(ctx context.Context, id, topicID int64, content string) (*PostModel.Post, error) {
	req := kafkaRequest{
		Action:    "update",
		ID:        id,
		topicID:   topicID,
