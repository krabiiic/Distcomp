...
import (
	...
	"lab3/discussion/pkg/keylock"
)

type KafkaConsumer struct {
	reader        *kafka.Reader
	writer        *kafka.Writer
	noticeService service.PostService
	locks         *keylock.KeyLock
}

func NewKafkaConsumer(brokers []string, groupID string, noticeService service.PostService) *KafkaConsumer {
...
func (k *KafkaConsumer) processPost(ctx context.Context) {
...
func (k *KafkaConsumer) processRequest(ctx context.Context, request struct {
	...
	case "create":
		notice, err := k.noticeService.CreatePost(ctx, model.Post{
			TopicID: request.IssueID,
	...
	case "get":
		notice, err := k.noticeService.GetPost(ctx, request.ID)
		if err != nil {
			response = map[string]interface{}{"error": err.Error()}
		} else {
			response = map[string]interface{}{"message": notice}
	...
	case "get_all":
		notices, err := k.noticeService.GetPosts(ctx)
		if err != nil {
			response = map[string]interface{}{"error": err.Error()}
		} else {
	...
	case "update":
		notice, err := k.noticeService.UpdatePost(ctx, model.Post{
			ID:      request.ID,
			TopicID: request.IssueID,
			Content: request.Content,
		})
	...
	case "delete":
		err := k.noticeService.DeletePost(ctx, request.ID)
		if err != nil {
			response = map[string]interface{}{"error": err.Error()}
		} else {
