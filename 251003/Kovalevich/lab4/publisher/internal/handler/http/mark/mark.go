...
import (
	...
	markModel "lab3/publisher/internal/model"
	markDbModel "lab3/publisher/internal/storage/psql/mark"
...
func (m Mark) InitRoutes(r *mux.Router) {
	r.HandleFunc("/marks", m.getMarksList).Methods(http.MethodGet)
	r.HandleFunc("/marks", m.createMark).Methods(http.MethodPost)
	r.HandleFunc("/marks/{id}", m.getMarkByID).Methods(http.MethodGet)
	r.HandleFunc("/marks/{id}", m.deleteMarkByID).Methods(http.MethodDelete)
	r.HandleFunc("/marks", m.updateMarkByID).Methods(http.MethodPut)
}
...
func (m Mark) getMarkByID(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if err == markDbModel.ErrpostNotFound {
...
func (m Mark) updateMarkByID(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewEncoder(w).Encode(updatedMark); err != nil {
		http.Error(w, fmt.Sprintf("failed to encode updated post: %v", err), http.StatusInternalServerError)
	}
}
